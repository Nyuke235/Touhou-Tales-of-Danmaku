...
export class JellybeanBullet extends RotatedBullet {
	constructor(x: number, y: number, vx: number, vy: number, color: BulletColor = 'purple') {
		const src = JELLYBEAN_SPRITES[color as keyof typeof JELLYBEAN_SPRITES] ?? JELLYBEAN_SPRITES.purple;
